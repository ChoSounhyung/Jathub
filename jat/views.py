...
def add_introduction(request, repository_pk):   # return render(request, '템플릿 이름', 그 템플릿에 넘겨주는 context)
    if request.method == 'POST':    # POST라면
        ...
        if form.is_valid():     #   그 정보가 확인되면 (valid())
            form.save()         #       DB에 저장
        ...
        if introduction == None:
            ...
            version = introduction.version + 1    #       repository에 있는 introduction중 가장 큰 버전 + 1
            contents = introduction.contents      #       introduction중 가장 큰 버전의 contents를 가져오기
            access = introduction.access

        initial = {'repository': repository, 'version': version, 'contents': contents, 'access': access}
        form = IntroductionForm(initial=initial)     #   form 가져오기
        context = {'form': None, 'repository': repository}        #   context = form, repository
